...
def combine_allure(folder):
    ...
    with open(folder + f"{sep}allure-report.html", "w", encoding="utf8") as f:
        f.write(str(soup))

    print(f"> Saving result as {folder}{sep}allure-report.html")

    size = os.path.getsize(folder + f'{sep}allure-report.html')
